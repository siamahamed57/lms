...
// Get current page from pretty URL or fallback
...
<body data-theme="dark">
            ...
            <div class="flex items-center space-x-6">
                ...
                <div class="hidden lg:flex items-center space-x-2 relative user-profile cursor-pointer group-hover-menu">
                    ...
                    <ul id="profile-dropdown" class="dropdown-menu absolute top-full right-0 mt-4 w-56 rounded-xl overflow-hidden transition-all duration-300 transform scale-95 origin-top-right z-20">
                        <?php if ($is_logged_in): ?>
                            <li><a href="/student/dashboard" class="block px-6 py-3 **text-card-color** hover:bg-card-hover-bg transition-colors duration-200"><i class="fas fa-th-large mr-2 text-[#b915ff]"></i> Dashboard</a></li>
                            <li><a href="/student/courses" class="block px-6 py-3 **text-card-color** hover:bg-card-hover-bg transition-colors duration-200"><i class="fas fa-book-open mr-2 text-[#60a5fa]"></i> My Courses</a></li>
                            <li><a href="/student/certificates" class="block px-6 py-3 **text-card-color** hover:bg-card-hover-bg transition-colors duration-200"><i class="fas fa-award mr-2 text-yellow-500"></i> Certificates</a></li>
                            <li><a href="/student/profile" class="block px-6 py-3 **text-card-color** hover:bg-card-hover-bg transition-colors duration-200"><i class="fas fa-cog mr-2 text-gray-400"></i> Profile Settings</a></li>
    ...
    <div id="mobile-menu-container" class="mobile-menu-container fixed top-0 right-0 w-80 max-w-full h-full bg-card-bg shadow-2xl z-[100] transform translate-x-full transition-transform duration-500 ease-in-out px-6 py-8">
        ...
        <ul class="space-y-6 text-xl">
            ...
            <?php if ($is_logged_in): ?>
                ...
                <li><a href="/student/courses" class="block **text-card-color** hover:text-[#b915ff] font-medium transition-colors duration-200"><i class="fas fa-book-open mr-3 text-sm text-[#60a5fa]"></i> My Courses</a></li>
            ...
            <?php else: ?>
                <li class="border-t pt-6 mt-6 border-gray-700"></li>
                <li><a href="/api/auth/login.php" class="block **text-card-color** hover:text-[#b915ff] font-medium transition-colors duration-200"><i class="fas fa-sign-in-alt mr-3 text-sm text-green-500"></i> Login</a></li>
                <li><a href="/api/auth/register.php" class="block **text-card-color** hover:text-[#b915ff] font-medium transition-colors duration-200"><i class="fas fa-user-plus mr-3 text-sm text-[#60a5fa]"></i> Register</a></li>
            <?php endif; ?>
        </ul>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Desktop Dropdowns (GSAP)
            const coursesTrigger = document.querySelector('.group-hover-menu .dropdown-toggle');
            ...
            // GSAP timelines for each dropdown
            const coursesTimeline = gsap.timeline({ paused: true, reversed: true });
            coursesTimeline.to(coursesMenu, {
                opacity: 1,
                scale: 1,
                visibility: 'visible',
                pointerEvents: 'auto',
            ...
            // Courses Dropdown Hover Logic
            coursesTrigger.parentNode.addEventListener('mouseenter', () => {
                clearTimeout(coursesTimeout);
                coursesTimeline.play();
            });
            coursesTrigger.parentNode.addEventListener('mouseleave', () => {
                coursesTimeout = setTimeout(() => coursesTimeline.reverse(), 200);
            });
